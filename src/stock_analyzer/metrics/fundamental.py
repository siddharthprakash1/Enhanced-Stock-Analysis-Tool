...
def compute_fundamental(f: Fundamentals, as_of: date) -> dict[str, MetricValue]:
    ...
    return {
        ...
        "roe": mv("roe", "Return on Equity", _pct(f.roe), "%"),
        "eps_growth": mv("eps_growth", "EPS Growth (Q)", _pct(f.eps_growth), "%"),
        "profit_margin": mv("profit_margin", "Profit Margin", _pct(f.profit_margin), "%"),
        "revenue_growth": mv("revenue_growth", "Revenue Growth", _pct(f.revenue_growth), "%"),
        "dividend_yield": mv("dividend_yield", "Dividend Yield", _pct(f.dividend_yield), "%"),
